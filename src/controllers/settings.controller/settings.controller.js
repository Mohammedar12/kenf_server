...
const Brand = require('../../models/settings.model/brand.model');
...
const createBrand = catchAsync(async (req, res, next) => {
    const body = pick(req.body, ['name_en', 'name_ar','active','images']);
    try{
        if(body.images){
            const validImages = await _validateUploads(body.images);
            if(!validImages){
...
const updateBrand = catchAsync(async (req, res, next) => {
    ...
    try{
        ...
        const update = await Brand.updateOne({ _id: convertObjectId(BrandId) },body);
        if(update.nModified == 0){
            return res.status(404).json({
                status: 404,
...
const getBrandList = catchAsync(async (req, res, next) => {
    const filter = pick(req.query, ['isKenf']);
    const options = pick(req.query, ['sort', 'limit', 'page']);
    if(!options.sort || options.sort === ''){
        options.sort = "-createdAt";
    ...
    if(req.session?.user?.role === 'admin'){
        options.select = "id name_en name_ar images active createdAt";
    }
    else{
        filter.active = true;
        options.select = "id name_en name_ar images ";
    }
    options.populate = {
        path: 'images',
        select: { link: 1, file_type: 1 }
...
const deleteBrand = catchAsync(async (req, res, next) => {
    const BrandId = req.params.id;
    try{
        const Brands = await Brand.findOne({ _id: convertObjectId(BrandId) },'id images');
        if(!Brands){
            return res.status(404).json({
                status: 404,
                message: 'Brand not found',
        ...
        await Brand.deleteOne({ _id: Brands.id });
        return res.status(200).json({
            status: 200,
            message: 'Item Category deleted successfully.',
        });
...
const getBrand = catchAsync(async (req, res, next) => {
    try {
        const brandId = req.params.id;
        const groupId = req.params.group_id;
        let brand = await Brand.findOne({ _id: convertObjectId(brandId),...(req.user?.role !== 'admin' ? {active: true} : {} ) }).populate('images','id link');
        if(!brand){
            return res.status(404).json({
                status: 404,
                message: 'Brand not found',
